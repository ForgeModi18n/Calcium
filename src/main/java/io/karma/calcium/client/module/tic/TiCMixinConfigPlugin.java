...
public final class TiCMixinConfigPlugin implements IMixinConfigPlugin {
    @Override
    ...
    public boolean shouldApplyMixin(final @NotNull String targetClassName, final @NotNull String mixinClassName) {
        if (!mixinClassName.startsWith("io.karma.calcium.client.module.tic.mixins")) {
            return true;
        }
        ...
        try { // @formatter:off
            return FMLLoader.getLoadingModList()
                ...
                .anyMatch(f -> f.getMods()
                    .stream()
                    .anyMatch(i -> i.getModId().equals("tconstruct")));
        } // @formatter:on
